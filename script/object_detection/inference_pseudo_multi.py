...
PATH_TO_LABELS = (
    f"{TRAINED_MODEL_DIR}/{MODEL_DIR_NAME}/training/labelmap.pbtxt"
)
...
NO_DETECTION_DIR = (
    f"{TRAINED_MODEL_DIR}/{MODEL_DIR_NAME}/pseudo/output/no_detection"
)
...
def copyImageIntoClassesFolder(
    targetName, imageOutputDir, image_file
):
...
def xml_inference_multi(model, image_file):
    ...
    h, w, c = img.shape
    if c == 1:
        img_rgb = cv2.cvtColor(img, cv2.COLOR_GRAY2RGB).astype(
            np.float32
        )
    else:
        img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB).astype(
            np.float32
        )
    img_rgb = img
    ...
    if voc_labels["all"]:
        ...
        if num_of_classes_found == 1:
            ...
            imageOutputFilename = copyImageIntoClassesFolder(
                object_name, OUTPUT_DIR, image_file
            )
            xml_gen.writeIntoXml(
                imageOutputFilename, voc_labels[object_name]
            )
        else:
            # save into mixed folder
            imageOutputFilename = copyImageIntoClassesFolder(
                "mixed_label", OUTPUT_DIR, image_file
            )
            xml_gen.writeIntoXml(
                imageOutputFilename, voc_labels["all"]
            )
    else:
        pass
...
def main():
    for image_file in IMAGE_PATHS:
        start_time = time.time()
        print(
            "Running inference for {}".format(image_file) + "\n",
            end="",
        )
